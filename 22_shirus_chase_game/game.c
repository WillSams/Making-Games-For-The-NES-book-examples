...
void show_screen(unsigned char num)
{
    ...
    if (num < LEVELS_ALL)
        ...
        spr = num - LEVELS_ALL + 1; // get offset in the screens list
...
void player_move(unsigned char id, unsigned char dir)
{
    ...
    if (map[MAP_ADR(px, py)] == TILE_WALL)
        return;
...
void game_loop(void)
{
    ...
    while (!game_done)
    {
        ...
        for (i = 0; i < player_all; ++i)
        {
            ...
            if (player_wait[i])
            {
                if (player_wait[i] >= 16 || player_wait[i] & 2)
                    py = 240;
            }
        ...
        if (!(frame_cnt & 3))
        {
            if (!game_paused && bright < 4)
                ++bright;
            if (game_paused && bright > 2)
                --bright;
        ...
        if (wait)
        {
            ...
            if (!wait)
                music_play(MUSIC_GAME); // start the music when all the objects spawned
        }
        ...
        for (i = 0; i < player_all; ++i)
        {
            ...
            if (wait)
                continue; // don't process object movements if spawn animation is running
            ...
            if (player_cnt[i])
            {
                ...
                {
                case DIR_RIGHT:
                    player_x[i] += player_speed[i];
                    break;
                case DIR_LEFT:
                    player_x[i] -= player_speed[i];
                    break;
                case DIR_DOWN:
                    player_y[i] += player_speed[i];
                    break;
                case DIR_UP:
                    player_y[i] -= player_speed[i];
                    break;
            ...
            if (!player_cnt[i]) // movement to the next tile is done, set up new movement
            {
                if (!i) // this is the player, process controls
                {
                    ...
                    if (j & PAD_LEFT)
                        player_move(i, DIR_LEFT);
                    if (j & PAD_RIGHT)
                        player_move(i, DIR_RIGHT);
                    if (j & PAD_UP)
                        player_move(i, DIR_UP);
                    if (j & PAD_DOWN)
                        player_move(i, DIR_DOWN);
                }
                else // this is an enemy, run AI
                {
                    ...
                    if (ptr != DIR_RIGHT && map[i16 - 1] != TILE_WALL)
                        dir[j++] = DIR_LEFT;
                    if (ptr != DIR_LEFT && map[i16 + 1] != TILE_WALL)
                        dir[j++] = DIR_RIGHT;
                    if (ptr != DIR_DOWN && map[i16 - MAP_WDT] != TILE_WALL)
                        dir[j++] = DIR_UP;
                    if (ptr != DIR_UP && map[i16 + MAP_WDT] != TILE_WALL)
                        dir[j++] = DIR_DOWN;
                    ...
                    if (j > 1)
                    {
                        if (ptr != DIR_DOWN && player_y[0] < player_y[i])
                            player_move(i, DIR_UP);
                        if (ptr != DIR_UP && player_y[0] > player_y[i])
                            player_move(i, DIR_DOWN);
                        if (ptr != DIR_RIGHT && player_x[0] < player_x[i])
                            player_move(i, DIR_LEFT);
                        if (ptr != DIR_LEFT && player_x[0] > player_x[i])
                            player_move(i, DIR_RIGHT);
                    }
                }
            }
